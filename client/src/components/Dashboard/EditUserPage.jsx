...
const EditUserPage = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        alert("Failed to load user. Redirecting to users list.");
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      // Don't send employeeId, rfid, or rfidTag to backend - they're auto-generated
      const { ...userData } = user;

      // Normalize role to lowercase for backend validation
      if (userData.role) {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      if (!width || !height || width <= 0 || height <= 0) {
        alert("Invalid card dimensions. Please try again.");
        return;
      }
  ...
  const downloadFrontPNG = async () => {
    try {
      ...
      a.click();
      alert("Front card exported successfully!");
    } catch (err) {
