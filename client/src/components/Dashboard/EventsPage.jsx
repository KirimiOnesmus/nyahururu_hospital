...
const EventsPage = () => {
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      notify.error(err.response?.data?.message || "Failed to fetch events");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useSocket("events:created", fetchEvents);
  useSocket("events:updated", fetchEvents);
  useSocket("events:deleted", fetchEvents);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.date)               { notify.error("Date is required"); return; }
    if (!formData.venue.trim())       { notify.error("Venue is required"); return; }
    ...
    try {
      ...
      notify.error(err.response?.data?.message || "Error saving event");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      notify.error(err.response?.data?.message || "Error deleting event");
    }
  };
    ...
    <div className="min-h-screen bg-[#f8f7f5]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        ...
        {loading ? (
          ...
          <EmptyState text={search ? "No events match your search" : "No events yet"} icon={FaCalendarAlt} />
        ) : (
          <>
            <div className="hidden md:block">
              <DataTable
                columns={[
                  { key: "banner", label: "Banner", render: (ev) => <Thumb url={ev.imageUrl} title={ev.title} /> },
                  {
                  ...
                  {
                    key: "date", label: "Date",
                    render: (ev) => <span className="flex items-center gap-1.5 text-xs text-gray-600"><FaCalendarAlt className="text-gray-300 shrink-0" />{fmtDate(ev.date)}</span>,
                  },
                  {
                    key: "venue", label: "Venue",
                    render: (ev) => <span className="flex items-center gap-1.5 text-xs text-gray-600"><FaMapMarkerAlt className="text-gray-300 shrink-0" />{ev.venue || ev.location || "—"}</span>,
                  },
                  {
                    key: "actions", label: "Actions", align: "right",
                    render: (ev) => (
                      <div className="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openView(ev)} className="p-2 rounded-xl text-blue-500 hover:bg-blue-50 cursor-pointer transition-colors" title="View"><FaEye className="text-sm" /></button>
                        <button onClick={() => openModal(ev)} className="p-2 rounded-xl text-gray-500 hover:bg-gray-100 cursor-pointer transition-colors" title="Edit"><MdEdit className="text-sm" /></button>
                        <button onClick={() => handleDelete(ev.id)} className="p-2 rounded-xl text-rose-400 hover:bg-rose-50 cursor-pointer transition-colors" title="Delete"><MdDelete className="text-sm" /></button>
                      </div>
                    ),
                  },
                ]}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <FormField label="Description" required>
            <textarea
              ...
              rows={5} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-shadow bg-white resize-none" required
            />
          </FormField>
