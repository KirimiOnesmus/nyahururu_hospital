...
const STEP_FIELDS = [
  ...
  ["researchProgramme", "sdg", "studyImplementationCounties",
   "protocolVersionNumber", "justification"],                        // step 1 — SERU Protocol Info
  ["background", "objectives", "methodology", "expectedOutcome",
   "inclusionCriteria", "exclusionCriteria", "sampleSizeDescription",
   "ethicsHumanSubjects"],                                           // step 2 — Methodology & Ethics
  ["file"],                                                          // step 3 — Documents
  [],                                                                // step 4 — Review & Payment
];

const STEPS = [
  { label: "Study Details",        sub: "Step 1" },
  { label: "SERU Protocol Info",   sub: "Step 2" },
  { label: "Methodology & Ethics", sub: "Step 3" },
  { label: "Documents & Budget",   sub: "Step 4" },
  { label: "Review & Payment",     sub: "Step 5" },
];
...
const CoInvestigators = ({ list, onChange, error }) => {
  ...
  const addEntry = () => {
    if (!draft.name.trim() || !draft.role.trim() || !draft.email.trim()) return;
    onChange([...list, draft]);
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <p className="text-xs text-slate-400 -mt-0.5">
        Co-investigators will receive an invitation email and get read access to this study.
      </p>
          ...
          {list.map((p, i) => (
            <div key={`${p.name}-${i}`}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 truncate">{p.role} · {p.email}</p>
              </div>
      ...
      <div className="grid sm:grid-cols-3 gap-2 mt-1">
        ...
        <input type="email" value={draft.email} onChange={(e) => setDraft((d) => ({ ...d, email: e.target.value }))}
          placeholder="Email (required)" className={inputCls(false)} />
      </div>
      <button type="button" onClick={addEntry}
        disabled={!draft.name.trim() || !draft.role.trim() || !draft.email.trim()}
        className="self-start flex items-center gap-1.5 text-xs font-semibold text-blue-600
          hover:underline transition-colors cursor-pointer mt-1 disabled:opacity-40 disabled:cursor-not-allowed">
...
const StepSeruProtocol = ({ form, setField, errors }) => (
  <div className="flex flex-col gap-5">
    <div className="grid sm:grid-cols-2 gap-4">
      <Field label="Research Programme" required error={errors.researchProgramme}>
        <select value={form.researchProgramme} onChange={(e) => setField("researchProgramme", e.target.value)}
          className={inputCls(errors.researchProgramme)}>
          <option value="" disabled>Select programme…</option>
          {KEMRI_PROGRAMMES.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
      </Field>
      <Field label="Sustainable Development Goal" error={errors.sdg}>
        <input value={form.sdg} onChange={(e) => setField("sdg", e.target.value)}
          placeholder="e.g. SDG 3 — Good Health and Well-being" className={inputCls(errors.sdg)} />
      </Field>
    </div>

    <div className="grid sm:grid-cols-2 gap-4">
      <Field label="Key Performance Area" error={errors.keyPerformanceArea}>
        <input value={form.keyPerformanceArea} onChange={(e) => setField("keyPerformanceArea", e.target.value)}
          placeholder="e.g. Disease Surveillance" className={inputCls(false)} />
      </Field>
      <Field label="Strategy" error={errors.strategy}>
        <input value={form.strategy} onChange={(e) => setField("strategy", e.target.value)}
          placeholder="e.g. Health Systems Strengthening" className={inputCls(false)} />
      </Field>
    </div>

    <div className="grid sm:grid-cols-2 gap-4">
      <Field label="Protocol Version Number" required error={errors.protocolVersionNumber}>
        <input value={form.protocolVersionNumber} onChange={(e) => setField("protocolVersionNumber", e.target.value)}
          placeholder="e.g. 1.0" className={inputCls(errors.protocolVersionNumber)} />
      </Field>
      <Field label="Protocol Version Date" error={errors.protocolVersionDate}>
        <input type="date" value={form.protocolVersionDate}
          onChange={(e) => setField("protocolVersionDate", e.target.value)}
          className={inputCls(false)} />
      </Field>
    </div>

    <Field label="Study Implementation Counties" required error={errors.studyImplementationCounties}
      hint={`${form.studyImplementationCounties.length} selected`}>
      <div className="max-h-40 overflow-y-auto border border-slate-200 rounded-xl p-3 grid grid-cols-2 sm:grid-cols-3 gap-1.5">
        {KENYAN_COUNTIES.map((county) => (
          <label key={county} className="flex items-center gap-1.5 text-xs cursor-pointer">
            <input type="checkbox"
              checked={form.studyImplementationCounties.includes(county)}
              onChange={(e) => {
                const next = e.target.checked
                  ? [...form.studyImplementationCounties, county]
                  : form.studyImplementationCounties.filter((c) => c !== county);
                setField("studyImplementationCounties", next);
              }}
    ...
    <Field label="Total Funds Needed (KES)" error={errors.totalFundsNeeded}>
      <input type="number" value={form.totalFundsNeeded}
        onChange={(e) => setField("totalFundsNeeded", e.target.value)}
        placeholder="e.g. 500000" className={inputCls(false)} />
    </Field>

    <Field label="Justification for Study" required error={errors.justification}
      hint="Why is this research important?">
      <textarea rows={4} value={form.justification}
        onChange={(e) => setField("justification", e.target.value)}
        placeholder="Emphasize how results will provide new knowledge and why it matters for national development…"
        className={`${inputCls(errors.justification)} resize-none`} />
    </Field>
  </div>
);


const StepMethodology = ({ form, setField, errors, acucFile, setAcucFile, insuranceFile, setInsuranceFile }) => {
  const fields = [
    ...
    <div className="flex flex-col gap-5">
      ...
      <Field label="Inclusion Criteria" error={errors.inclusionCriteria} hint="Who can participate?">
        <textarea rows={2} value={form.inclusionCriteria}
          ...
          placeholder="Criteria for inclusion of study subjects…"
          className={`${inputCls(errors.inclusionCriteria)} resize-none`} />
      </Field>

      <Field label="Exclusion Criteria" error={errors.exclusionCriteria}>
        <textarea rows={2} value={form.exclusionCriteria}
          onChange={(e) => setField("exclusionCriteria", e.target.value)}
          placeholder="Criteria for exclusion of subjects…"
          className={`${inputCls(false)} resize-none`} />
      </Field>

      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="Sample Size" error={errors.sampleSizeDescription}>
          <textarea rows={2} value={form.sampleSizeDescription}
            onChange={(e) => setField("sampleSizeDescription", e.target.value)}
            placeholder="Sample size determination and justification…"
            className={`${inputCls(false)} resize-none`} />
        </Field>
        <Field label="Sampling Procedure" error={errors.samplingProcedure}>
          <textarea rows={2} value={form.samplingProcedure}
            onChange={(e) => setField("samplingProcedure", e.target.value)}
            placeholder="Random, stratified, convenience, etc."
            className={`${inputCls(false)} resize-none`} />
        </Field>
      </div>

      <Field label="Data Management Plan" error={errors.dataManagementPlan}
        hint="How data will be stored, analysed">
        <textarea rows={3} value={form.dataManagementPlan}
          onChange={(e) => setField("dataManagementPlan", e.target.value)}
          placeholder="Describe data storage, software, statistical methods, confidentiality measures…"
          className={`${inputCls(false)} resize-none`} />
      </Field>

      <Field label="Ethical Considerations — Human Subjects" error={errors.ethicsHumanSubjects}
        hint="Required for studies involving people">
        <textarea rows={3} value={form.ethicsHumanSubjects}
          onChange={(e) => setField("ethicsHumanSubjects", e.target.value)}
          placeholder="Informed consent process, confidentiality, risks/benefits to participants, do-no-harm measures…"
          className={`${inputCls(errors.ethicsHumanSubjects)} resize-none`} />
      </Field>

      <Field label="Ethical Considerations — Animal Subjects" error={errors.ethicsAnimalSubjects}
        hint="Leave blank if this study does not involve animal subjects">
        <textarea rows={3} value={form.ethicsAnimalSubjects}
          onChange={(e) => setField("ethicsAnimalSubjects", e.target.value)}
          placeholder="Species, housing/welfare provisions, ACUC oversight arrangements…"
          className={`${inputCls(errors.ethicsAnimalSubjects)} resize-none`} />
        {form.ethicsAnimalSubjects?.trim() && (
          <div className="mt-2">
            <label className="text-xs font-medium text-slate-600 mb-1 block">
              ACUC Approval Letter <span className="text-red-500">*</span>
            </label>
            <input type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
              onChange={(e) => setAcucFile(e.target.files[0] || null)}
              className="text-sm text-slate-600 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg
                file:border-0 file:bg-blue-50 file:text-blue-700 file:text-xs file:font-semibold" />
            {acucFile && <p className="text-xs text-emerald-600 mt-1">Selected: {acucFile.name}</p>}
            <p className="text-[11px] text-slate-400 mt-1">Required per SOP-1 Annex 3 for studies involving animal subjects.</p>
          </div>
        )}
      </Field>

      <Field label="Investigational Product">
        <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
          <input type="checkbox" checked={!!form.isInvestigationalProduct}
            onChange={(e) => setField("isInvestigationalProduct", e.target.checked)}
            className="w-4 h-4 rounded border-slate-300" />
          This study involves an investigational drug, device, or other IND-regulated product
        </label>
        {form.isInvestigationalProduct && (
          <div className="mt-2">
            <label className="text-xs font-medium text-slate-600 mb-1 block">
              Clinical Trial Insurance Certificate <span className="text-red-500">*</span>
            </label>
            <input type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
              onChange={(e) => setInsuranceFile(e.target.files[0] || null)}
              className="text-sm text-slate-600 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg
                file:border-0 file:bg-blue-50 file:text-blue-700 file:text-xs file:font-semibold" />
            {insuranceFile && <p className="text-xs text-emerald-600 mt-1">Selected: {insuranceFile.name}</p>}
            <p className="text-[11px] text-slate-400 mt-1">Required per SOP-1 §10.6 for investigational-product studies.</p>
          </div>
        )}
      </Field>

      <Field label="Budget Summary" error={errors.budgetSummary}>
        <textarea rows={3} value={form.budgetSummary}
          onChange={(e) => setField("budgetSummary", e.target.value)}
          placeholder="Major budget components: personnel, travel, equipment, supplies…"
          className={`${inputCls(false)} resize-none`} />
      </Field>
    </div>
  );
...
const ReviewRows = ({ form, file }) => {
  const rows = [
    ...
    ["Research Programme", form.researchProgramme],
    ["Protocol Version", form.protocolVersionNumber],
    ["SDG", form.sdg],
    ["Counties", form.studyImplementationCounties.join(", ")],
    ...
    ["Inclusion Criteria", form.inclusionCriteria],
    ["Ethics (Human)", form.ethicsHumanSubjects],
...
const StepReviewPayment = ({ form, file, acucFile, insuranceFile, onComplete, onServerError }) => {
  ...
  const buildPayload = () => ({
    ...
    // SERU fields
    researchProgramme: form.researchProgramme,
    keyPerformanceArea: form.keyPerformanceArea,
    strategy: form.strategy,
    sdg: form.sdg,
    protocolVersionNumber: form.protocolVersionNumber,
    protocolVersionDate: form.protocolVersionDate,
    studyImplementationCounties: form.studyImplementationCounties,
    totalFundsNeeded: form.totalFundsNeeded,
    expectedDurationMonths: form.duration,
    justification: form.justification,
    inclusionCriteria: form.inclusionCriteria,
    exclusionCriteria: form.exclusionCriteria,
    sampleSizeDescription: form.sampleSizeDescription,
    samplingProcedure: form.samplingProcedure,
    dataManagementPlan: form.dataManagementPlan,
    ethicsHumanSubjects: form.ethicsHumanSubjects,
    ethicsAnimalSubjects: form.ethicsAnimalSubjects,
    isInvestigationalProduct: form.isInvestigationalProduct,
    budgetSummary: form.budgetSummary,
    budgetJustification: form.budgetJustification,
    expectedApplicationOfResults: form.expectedApplicationOfResults,
    studySites: form.studySites,
    literatureReviewSummary: form.literatureReviewSummary,
  });
  ...
  const submitProposal = async (paymentId, mpesaReceipt) => {
    ...
    try {
      await confirmProposalSubmission(buildPayload(), paymentId, file, {
        acucApprovalDoc: acucFile,
        insuranceCertificateDoc: insuranceFile,
      });
      setStage("success");
      setTimeout(() => onComplete({ paymentId, receipt: mpesaReceipt }), 1800);
    } catch (err) {
      const { summary } = formatApiError(err);
      setError(`${summary} Your payment was received — click "Retry Submission" below.`);
      setStage("error");
      // Store IDs so the retry button can reuse them
      retryRef.current = { paymentId, mpesaReceipt };
    }
  };
  ...
  const handleRetrySubmission = () => {
    if (retryRef.current) {
      submitProposal(retryRef.current.paymentId, retryRef.current.mpesaReceipt);
    }
  };
  ...
  if (stage === "error") return (
    <div className="flex flex-col gap-3.5">
      ...
      {retryRef.current && (
        <button type="button" onClick={handleRetrySubmission}
          ...
          Retry Submission (payment already received)
        </button>
      )}
      <button type="button" onClick={() => { setStage("input"); setError(""); retryRef.current = null; }}
...
const SubmitProposal = ({ onClose, onSubmitted }) => {
  ...
  const [acucFile, setAcucFile] = useState(null); // F-14
  const [insuranceFile, setInsuranceFile] = useState(null); // F-14
  ...
  const validate = (s) => {
    ...
    if (s === 1) {
      if (!form.researchProgramme) e.researchProgramme = "Select a research programme";
      if (!form.protocolVersionNumber.trim()) e.protocolVersionNumber = "Protocol version number is required";
      if (!form.studyImplementationCounties.length) e.studyImplementationCounties = "Select at least one county";
      if (!form.justification.trim()) e.justification = "Study justification is required";
    }
    ...
    if (s === 3 && !file) e.file = "Please upload your proposal PDF";
  ...
  const stepContent = [
    ...
    <StepSeruProtocol form={form} setField={setField} errors={errors} />,
    <StepMethodology form={form} setField={setField} errors={errors}
      acucFile={acucFile} setAcucFile={setAcucFile}
      insuranceFile={insuranceFile} setInsuranceFile={setInsuranceFile} />,
    <StepDocuments file={file} onFile={setFile} onClear={() => setFile(null)} error={errors.file} />,
    <StepReviewPayment form={form} file={file} acucFile={acucFile} insuranceFile={insuranceFile}
      onComplete={handleComplete} onServerError={handleServerValidationError} />,
  ];
