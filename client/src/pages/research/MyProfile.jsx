...
const userRole =
  ROLE_ALIAS[(localStorage.getItem("role") || "").toLowerCase()] ||
  "researcher";
...
const EditProfileModal = ({ profile, onClose, onSave }) => {
  ...
  const inputCls =
    "w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-800 outline-none focus:ring focus:ring-blue-500 bg-gray-50 hover:border-gray-300 transition-all";
      ...
      >
        ...
        <div className="bg-blue-600 px-6 py-4 flex items-center justify-between flex-shrink-0">
          ...
          {!saving && (
            <button
              onClick={onClose}
              className="text-white/60 hover:text-red-600 
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setProfile(profileRes.researcher);
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col h-full">
        <div className="bg-blue-500 px-8 py-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div
              className="w-11 h-11 bg-white/20 rounded-xl flex
